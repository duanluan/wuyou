...
CREATE TABLE `gen_table`  (
  ...
  `gen_type` tinyint(1) NULL DEFAULT '0' COMMENT '生成代码方式（0: zip 压缩包, 1: 自定义路径）',
  `gen_path` varchar(200) NULL DEFAULT '/' COMMENT '生成路径（不填默认项目路径）',
  `options` varchar(1000) NULL DEFAULT NULL COMMENT '其他生成选项',
...
) ENGINE = InnoDB AUTO_INCREMENT = 1 COMMENT = '代码生成业务表字段' ROW_FORMAT = DYNAMIC;
...
) ENGINE = InnoDB AUTO_INCREMENT = 5 COMMENT = '参数配置表' ROW_FORMAT = DYNAMIC;
...
) ENGINE = InnoDB AUTO_INCREMENT = 11 COMMENT = '字典类型表' ROW_FORMAT = DYNAMIC;
...
) ENGINE = InnoDB AUTO_INCREMENT = 1 COMMENT = '定时任务调度日志表' ROW_FORMAT = DYNAMIC;
...
CREATE TABLE `sys_logininfor`  (
  ...
  `ipaddr` varchar(50) NULL DEFAULT '' COMMENT '登录IP地址',
...
) ENGINE = InnoDB AUTO_INCREMENT = 1062 COMMENT = '菜单权限表' ROW_FORMAT = DYNAMIC;
...
CREATE TABLE `sys_notice`  (
  ...
  `notice_content` varchar(2000) NULL DEFAULT NULL COMMENT '公告内容',
...
) ENGINE = InnoDB AUTO_INCREMENT = 2 COMMENT = '操作日志记录' ROW_FORMAT = DYNAMIC;
...
) ENGINE = InnoDB AUTO_INCREMENT = 10 COMMENT = '角色信息表' ROW_FORMAT = DYNAMIC;
...
) ENGINE = InnoDB COMMENT = '在线用户记录' ROW_FORMAT = DYNAMIC;
